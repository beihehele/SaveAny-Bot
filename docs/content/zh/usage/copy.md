...
将源频道/群的历史消息按条件筛选后，经 UserBot **无来源复制**（`DropAuthor`）到目标聊天（可指定论坛话题），并在消息开头加上可点击的 `[转]` 链接。源消息删除不影响已复制内容。不下载、不落本地存储。
...
- 命中后 `forwardMessages`（DropAuthor）；相册整组转发，再编辑 caption 在开头加上可点击 `[转]`
